"""
09_evaluation_week2.py
======================
Valutazione della Settimana 2 (modello addestrato su Flickr30k) sui DUE test set.

Lo script:
  1. carica il best checkpoint del training Flickr30k (script 08);
  2. lo valuta sia sul test di Flickr8k sia sul test di Flickr30k, calcolando BLEU;
  3. costruisce tabelle comparative con tutti gli esperimenti precedenti;
  4. verifica l'assenza di leakage nello split di Flickr30k;
  5. fa pulizia dei checkpoint intermedi non necessari (libera spazio disco).

La doppia valutazione (F8k e F30k) e' il punto chiave: confrontare lo stesso
modello su un test "in-distribution" (F30k) e uno "cross-distribution" (F8k)
mette in luce le differenze di generalizzazione.
"""
...
INITIAL_MODEL_DIR = CHECKPOINT_DIR / "model_initial"            # tokenizer + image_processor
BEST_MODEL_DIR = CHECKPOINT_DIR / "week2_flickr30k" / "best"     # best del training Flickr30k (script 08)
...
# ----------------------------------------------------------------------------
# 3. TRASFORMAZIONI DI VALUTAZIONE
# ----------------------------------------------------------------------------
# Identiche a quelle usate in training/eval (deterministiche, senza augmentation).
...
# ----------------------------------------------------------------------------
# 4. FUNZIONI DI GENERAZIONE E VALUTAZIONE
# ----------------------------------------------------------------------------
@torch.no_grad()
def generate_caption_batch(model, pixel_values_batch, tokenizer, device):
    """Genera didascalie per un batch di immagini (beam search).
    Workaround GPT-2 BOS/EOS: decoder_input_ids=[[BOS,BOS]] per esempio."""
...
def evaluate_on_test_set(model, test_df, images_dir, tokenizer, device, batch_size=16, desc="Test"):
    """Genera le didascalie per tutte le immagini del test set e calcola il BLEU.

    Funzione generica riutilizzabile su qualunque test set (F8k o F30k):
    basta passare il test_df e la cartella immagini corrispondente.
    """
    ...
    # Iterazione a batch: carica, trasforma, genera.
    ...
    # BLEU con reference trasposte (formato sacrebleu).
    ...
    # Restituiamo metriche + le didascalie grezze (utili per ispezioni successive).
    return {
...
# ----------------------------------------------------------------------------
# 5. EVALUATION 1: TEST FLICKR8K (cross-distribution: train F30k -> test F8k)
# ----------------------------------------------------------------------------
...
# Salviamo solo le metriche (non le didascalie grezze) nel JSON.
...
# ----------------------------------------------------------------------------
# 6. EVALUATION 2: TEST FLICKR30K (in-distribution: train F30k -> test F30k)
# ----------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------
# 7. TABELLA COMPARATIVA (versione rapida, da risultati in memoria)
# ----------------------------------------------------------------------------
# Raccoglie i risultati di tutti gli esperimenti fatti finora. I primi due
# (baseline e Scenario B) vengono letti dai file salvati, se presenti.
...
# Settimana 2: aggiunge le due righe appena calcolate (test F8k e test F30k).
experiments.append({
...
# ----------------------------------------------------------------------------
# 8. VERIFICA NO LEAKAGE NELLO SPLIT DI FLICKR30K
# ----------------------------------------------------------------------------
# Doppio controllo (oltre agli assert dello script 07): rileggiamo lo split di
# Flickr30k e verifichiamo che gli insiemi di immagini dei tre split siano disgiunti.
...
# Insiemi di immagini per split; le intersezioni devono essere tutte vuote.
...
# ----------------------------------------------------------------------------
# 9. PULIZIA CHECKPOINT INTERMEDI (libera spazio disco)
# ----------------------------------------------------------------------------
# Rimuove checkpoint non piu' necessari: l'epoca intermedia dello Scenario A e i
# "last" dei due training (teniamo solo i "best"). ATTENZIONE: operazione
# distruttiva e irreversibile; eseguire solo a esperimenti completati.
import shutil
...
for path in to_delete:
    if path.exists():
        # Calcola lo spazio liberato prima di cancellare (solo per il log).
...
# ----------------------------------------------------------------------------
# 10. TABELLA COMPARATIVA FINALE (da file JSON, anche in Markdown per il report)
# ----------------------------------------------------------------------------
# Versione definitiva della tabella: legge tutte le metriche dai file salvati e
# produce sia una stampa a video sia un file Markdown pronto per la relazione.
files = {
...
for label, path in files.items():
    ...
    # I vari JSON usano nomi di campo leggermente diversi (es. "BLEU-1" vs "BLEU_1"):
    # questa helper prova piu' chiavi e restituisce la prima presente.
    def get(d, *keys):
        for k in keys:
            if k in d:
                return d[k]
...
# Salvataggio in Markdown, comodo da incollare nel report.
